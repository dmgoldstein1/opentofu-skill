...
| **Cross-cloud / provider mapping** | "What's the Azure/GCP/Vultr equivalent of X", picking a backend/auth model per cloud | [State Management: Cross-cloud equivalents](references/state-management.md#cross-cloud-equivalents), [Vultr Provider Patterns](references/vultr-provider.md) |
...
**Do:** source secrets from a cloud secret manager (AWS Secrets Manager / Azure Key Vault / GCP Secret Manager), or inject provider credentials via CI environment variables (for example `VULTR_API_KEY`), or use `write_only` arguments on 1.11+; create dedicated VPCs, enforce encryption at rest and TLS, least-privilege SGs, and use separate `aws_vpc_security_group_{ingress,egress}_rule` resources (e.g. AWS provider v5+).
...
AWS example (Azure `azurerm` / GCP `gcs` / Vultr object storage via `s3`-compatible backend patterns / hosted backend syntax: see [State Management: Choosing a Remote Backend](references/state-management.md#choosing-a-remote-backend) and [Vultr Provider Patterns](references/vultr-provider.md)):
...
- [Vultr Provider Patterns](references/vultr-provider.md) — provider setup, resource families, Vultr-specific examples
