...
|---------|-----|-------|-----|-------|
| Secret manager / secret input | `aws_secretsmanager_secret` | `azurerm_key_vault_secret` | `google_secret_manager_secret` | Inject `VULTR_API_KEY` and other secrets from CI/secret manager; avoid defaults and committed `.tfvars` |
| Network firewalling | `aws_security_group` + `aws_vpc_security_group_*_rule` | `azurerm_network_security_group` + `azurerm_network_security_rule` | `google_compute_firewall` | `vultr_firewall_group` + `vultr_firewall_rule` |
| Identity | IAM (`aws_iam_role` / `aws_iam_policy`) | RBAC (`azurerm_role_assignment`) | IAM (`google_project_iam_*`) | `organization_*`, `oidc_*`, and `user` resources |
| Encryption at rest | explicit (SSE / KMS) | default-on (optional CMK) | default-on (optional CMEK) | Use Vultr object-storage and managed-service encryption controls for each service |
