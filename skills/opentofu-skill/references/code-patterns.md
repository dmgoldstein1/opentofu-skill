...
> **Purpose:** Comprehensive patterns for OpenTofu code structure and modern features

This document provides detailed code patterns, structure guidelines, and modern OpenTofu features. For high-level principles, see the [main skill file](../SKILL.md).
...
3. [Modern OpenTofu Features](#modern-opentofu-features)
...
# Verify migration:
# tofu plan should show "moved" operations, not destroy/create
```
...
| native `tofu test` | 1.6+ | treats mocked-provider tests as full integration coverage |
...
### optional() with Defaults (OpenTofu 1.3+)
...
### Cross-Variable Validation (OpenTofu 1.9+)
...
variable "storage_size" {
  ...
  validation {
    # Can reference var.instance_type in OpenTofu 1.9+
    condition = !(
...
### Write-Only Arguments (OpenTofu 1.11+)
...
resource "aws_db_instance" "this" {
  ...
  # or inject via CI env var outside OpenTofu.
...
### nonsensitive() and ephemeral (OpenTofu 0.15+ / 1.10+)
...
- ❌ No drift detection — OpenTofu can't observe what scripts changed
...
- Prioritize provider and OpenTofu runtime updates
...
- [ ] Migrate secrets to write-only arguments (OpenTofu 1.11+)
- [ ] Use `moved` blocks for resource refactoring (OpenTofu 1.1+)
- [ ] Consider cross-variable validation (OpenTofu 1.9+)
...
✅ GOOD — 1.11+ write-only argument, secret created outside OpenTofu:
...
Pre-1.11 fallback: use the same data source without `password_wo`; rotation must happen outside OpenTofu.

**Migration steps:**

1. Create secret in AWS Secrets Manager outside OpenTofu
...
# Without local: OpenTofu might try to delete CIDR before subnets → ERROR
