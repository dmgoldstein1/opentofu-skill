# CI/CD Workflows for OpenTofu

> **Part of:** [terraform-skill](../SKILL.md)
> **Purpose:** CI/CD integration patterns for OpenTofu on macOS and Linux runners
...
```yaml
# GitHub Actions — set TF_PLUGIN_CACHE_DIR so `tofu init` actually
  ...
  detect:
    ...
    steps:
      ...
      - name: Plan (detect drift)
        id: plan
        run: tofu plan -detailed-exitcode -out=plan.bin
...
[Atlantis](https://www.runatlantis.io/) provides OpenTofu automation via pull request comments.
...
**Cause:** Different OpenTofu/provider versions
...
- re-runs `tofu plan` inside the apply job instead of consuming the reviewed plan artifact from the plan stage
...
- implements scheduled "drift detection" as `tofu apply -auto-approve` on cron — silently reverts out-of-band changes; use `plan -detailed-exitcode` + alert
- fails to restrict artifact access when `tofu show -json` results may contain sensitive plan output
