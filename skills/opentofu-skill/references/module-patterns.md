...
| Resource | AWS | Azure | GCP | Vultr |
|----------|-----|-------|-----|-------|
| Network | `aws_vpc` | `azurerm_virtual_network` | `google_compute_network` | `vultr_vpc` / `vultr_vpc2` |
| Subnet | `aws_subnet` | `azurerm_subnet` | `google_compute_subnetwork` | VPC-managed network segmentation (provider-specific model) |
| Compute instance | `aws_instance` | `azurerm_linux_virtual_machine` | `google_compute_instance` | `vultr_instance` |
| Managed relational DB | `aws_db_instance` / `aws_rds_cluster` | `azurerm_*_flexible_server` | `google_sql_database_instance` | `vultr_database` |
| Object storage | `aws_s3_bucket` | `azurerm_storage_account` + `azurerm_storage_container` | `google_storage_bucket` | `vultr_object_storage` + `vultr_object_storage_bucket` |
