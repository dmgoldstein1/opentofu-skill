...
generic protocol; the OpenTofu-specific capability matrix, routing, rename
...
2. Enable the host agent's LSP tool support and reload the agent process.
  LSP tools usually require explicit opt-in by host-specific setting or env.
3. Install an LSP transport that exposes a POSITION-based terraform-ls tool
  (`filePath`, `line`, `character`). If your host exposes only name-based
  lookups, the transport is too old or misconfigured - reinstall and restart.
  Example for one host (optional): `boostvolt/claude-code-lsps` with
...
3. Apply the edit, then re-run `tofu validate` and check LSP diagnostics
   are clean.

**(b) Resource or module address** - this is NOT a text rename:

- Add a `moved` block and run `tofu plan`; confirm it shows 0 destroy /
