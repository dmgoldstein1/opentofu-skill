...
| `s3` (S3-compatible) | Vultr Object Storage and other S3-compatible object stores |
...
For Vultr provider infrastructure patterns (resource families, auth, and examples), see [Vultr Provider Patterns](vultr-provider.md).

| Concern | AWS | Azure | GCP | Vultr |
|---------|-----|-------|-----|-------|
| Backend block | `backend "s3" { bucket, key, region, encrypt, use_lockfile }` | `backend "azurerm" { resource_group_name, storage_account_name, container_name, key }` | `backend "gcs" { bucket, prefix }` | `backend "s3" { ... }` against Vultr Object Storage |
| Access control | IAM policy on bucket/role | RBAC role assignment on storage account/container | IAM binding on the bucket | Vultr object-storage key scoping + Vultr org/user policy controls |
| Remote-state data source | `terraform_remote_state` (backend `s3`) | `terraform_remote_state` (backend `azurerm`) | `terraform_remote_state` (backend `gcs`) | `terraform_remote_state` (backend `s3`) |

### Bootstrap parity

| Concern | AWS | Azure | GCP | Vultr |
|---------|-----|-------|-----|-------|
| Versioning | S3 bucket versioning | storage account / blob versioning | GCS object versioning | Object-storage versioning features per Vultr object storage capabilities |
| Encryption at rest | explicit: SSE / KMS (`aws_s3_bucket_server_side_encryption_configuration`) | default-on (SSE; optional CMK) | default-on (Google-managed; optional CMEK) | Object storage and managed-service encryption capabilities per Vultr provider/resources |
| Public-access block | `aws_s3_bucket_public_access_block` | `allow_nested_items_to_be_public = false` + private container | uniform bucket-level access + public access prevention | Private bucket policy and key scoping in Vultr object storage |
| Bootstrap auth | IAM / OIDC | RBAC / federated credentials | IAM / Workload Identity Federation | API key + org/user/oidc controls (`organization_*`, `oidc_*`) |
